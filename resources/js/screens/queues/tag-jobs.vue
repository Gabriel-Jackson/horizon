<script type="text/ecmascript-6">
    ...
    export default {
         ...
         */
        mounted() {
            document.title = "Api Produtos - Monitorando";
...
<template>
    <div>
        ...
        <div v-if="ready && jobs.length == 0" class="d-flex flex-column align-items-center justify-content-center card-bg-secondary p-5 bottom-radius">
            <span>Não há nenhum Job para essa fila</span>
        </div>
        ...
        <table v-if="ready && jobs.length > 0" class="table table-hover table-sm mb-0">
            ...
            <tr>
                <th>Job</th>
                <th>Adicionado em</th>
                <th v-if="type == 'jobs'">Tempo de Execução</th>
                <th class="text-right" v-if="type == 'jobs'">Status</th>
                <th class="text-right" v-if="type == 'failed'">Falhou em:</th>
            </tr>
            </thead>
        ...
        <div v-if="ready && jobs.length" class="p-3 d-flex justify-content-between border-top">
            <button @click="previous" class="btn btn-secondary btn-md" :disabled="page==1">Anterior</button>
            <button @click="next" class="btn btn-secondary btn-md" :disabled="page>=totalPages">Próximo</button>
        </div>
    </div>
